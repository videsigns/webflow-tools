...
if(savedFilledInput && memory){
  savedFilledInput.forEach(x =>{
    if($(`input[name="${x.inputName}"][value="${x.value}"]`).attr('type') === 'radio'){
      ...
      $(`input[name="${x.inputName}"][value="${x.value}"]`).siblings('.w-radio-input').addClass('w--redirected-checked')

    }else if(x.value === 'on'){
      $(`input[name="${x.inputName}"]`).click()
      $(`input[name="${x.inputName}"]`).siblings('.w-checkbox-input').addClass('w--redirected-checked')
    }else{
...
if(params){
  ...
  searchQ.forEach(y =>{
    console.log(y)
    if($(`input[name="${y.key}"][value="${x.val}"]`).attr('type') === 'radio'){
      $(`input[name="${y.key}"][value="${x.val}"]`).click()
      $(`input[name="${y.key}"][value="${x.val}"]`).siblings('.w-radio-input').addClass('w--redirected-checked')
  
    }else if(y.val === 'on'){
      $(`input[name="${y.key}"]`).click()
      $(`input[name="${y.key}"]`).siblings('.w-checkbox-input').addClass('w--redirected-checked')
    }else{
...
function updateStep() {
  ...
  if($('[data-clickable]').data('clickable')){
    ...
    steps.find(':input[required]').each(function(){
      $($('[data-form="custom-progress-indicator"]')[$(this).parents('[data-form="step"]').index()]).text($($('[data-form="custom-progress-indicator"]')[$(this).parents('[data-form="step"]').index()]).text().replace('*',''))
      if($(this).val() === ""){
        emptyInput++
  ...
  if(next){
    x = getSafe(() => selection[0]['skipTo']) ? parseInt(getSafe(() => selection[0]['skipTo'])) : x
  }
  ...
  } else if (x === steps.length - 1 || $(steps[x]).find('[data-form="submit"]:visible').length > 0) {
...
function validation() {
  ...
  if(!$('[data-logic-extra]').data('logic-extra')){
    ...
    if ($(steps[x]).find('[data-answer]:visible').find(':input').is('[type="checkbox"]')) {
      if (
        ...
        if (
          $(steps[x]).find('[data-answer]:visible').find(':input[type="checkbox"]:checked').length >= checkCount
        ) {
          //console.log($(steps[x]).find(':input[required').length);
          //if ($(steps[x]).find(':input[required]').length < 1) {

            if($(steps[x]).find('[data-answer]:visible').find(':input[type="checkbox"]:checked').parents('[data-go-to]').attr('data-go-to')){
              answer =  $(steps[x]).find('[data-answer]:visible').find(':input[type="checkbox"]:checked').parents('[data-go-to]').attr('data-go-to')
              selections = selections.filter(y => y.step !== x)
              selections.push({'step': x, 'selected':answer})
            }
            selections = selections.filter(y => y.step !== x)
    ...
    if ($(steps[x]).find('[data-answer]:visible').find(':input[required]').is('[type="radio"]')) {
      if ($(steps[x]).find('[data-answer]:visible').find(':input[type="radio"][required]').is(':checked')) {
        radioFilled = true;
      } else {
        radioFilled = false;
      }
  ...
  if (inputFilled && checkboxFilled && radioFilled && emailFilled && selectFilled && textareaFilled) {
    enableBtn();
  }else{
    disableBtn()
  }
...
function backStep() {
  if(x > 0 ){
    ...
    selections.filter(sk => sk.skipTo === x).length > 0 ? x = parseInt(getSafe(() => selections.filter(sk => sk.skipTo === x)[0].backTo )) : x--
...
$(steps)
  ...
  if ($(steps[x]).find(':input').is(':checked')) {
    ...
    selArr = []
    $(steps).find('[data-selected]:checked').each(function(y,i){     
    ...
    $(steps[x]).find('[data-answer]:visible').find(":input[type='radio']:checked").each(function(){
      if($(this).data('go-to')){
    ...
    if ($(steps[x]).find('[data-radio-skip]:visible').data('radio-skip') === true) {
      //console.log('skip')
      if(textareaLength === 0 && textInputLength === 0 && emailInputLength === 0 && checkboxInputLength === 0){
        setTimeout(function () {
          next = true
          nextStep();
          selectionQuiz()
...
if($('[data-form="multistep"]').data('debug-mode')){
  ...
  //data go to attr
  $('[data-go-to]').each(function(){$(this).append('<br>Data Go To = ', $(this).data('go-to'))})
  //data answer attr
  $('[data-answer]').each(function(){$(this).append('<br>Data Answer = ', $(this).data('answer'))})
}
/////////////////////////////
...
$('[data-form="submit-btn"]').on('click', function(e) {
  ...
  if($('[data-form="multistep"]').data('logic-extra')){
    if(curStep === $('[data-form="step"]:not([data-card="true"])').length || $(steps[x]).find('[data-form="submit"]:visible').length > 0){
  ...
  if($('[data-form="multistep"]').data('remove-unfilled')){
    for( j = 1; j <= selections.length; j++){
     $(steps[j]).find(`[data-answer]:not([data-answer="${selections[j-1].selected}"])`).remove()   
    }
  }
